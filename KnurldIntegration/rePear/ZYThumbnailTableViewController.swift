...
class ZYThumbnailTableViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UISearchResultsUpdating, UISearchBarDelegate, UIGestureRecognizerDelegate {
...
//MARK: PROPERTY
    ...
    */
    ...
    var newtableViewDataList = NSMutableArray()
    
    var indexPaths = NSMutableArray()
    /**
     your diy tableView cell ReuseIdentifier
    */
    var tableViewCellReuseId = "DIYTableViewCell"
    /**
    ...
    */
    ...
    private var newCellDictionary: NSMutableDictionary = NSMutableDictionary()
    /**
...
//MARK: FUNCTION
    override func viewDidLoad() {
        ...
        lpgr.delegate = self
        self.mainTableView.addGestureRecognizer(lpgr)
    }
    
    func handleLongPress(gestureRecognizer: UILongPressGestureRecognizer) {
        
        var p: CGPoint = gestureRecognizer.locationInView(self.mainTableView)
        var indexPath: NSIndexPath = mainTableView.indexPathForRowAtPoint(p)!
        
        let x = tableViewDataList.objectAtIndex(indexPath.row) as! Post
        //what to share
        defaults.setObject(x.content, forKey: "sharetext")
        
        if gestureRecognizer.state == .Began {
            NSLog("long press on table view at row %ld", indexPath.row)
            var selectedCell = cellDictionary.valueForKey("\(indexPath.row)") as? UITableViewCell
            ...
            NSLog("gestureRecognizer.state other")
        }
    ...
    func updateSearchResultsForSearchController(searchController: UISearchController) {
        ...
        for item in tableViewDataList{
            let title = item.name
            let range = title.rangeOfString(searchString!, options: NSStringCompareOptions.CaseInsensitiveSearch)
            if(range != nil){
                newtableViewDataList.addObject(item)
                indexPaths.addObject(tableViewDataList.indexOfObject(item))
            }
        }
        */
    ...
    */
    ...
    func tableView(tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if shouldShowSearchResults{
            return newtableViewDataList.count
        }
        return tableViewDataList.count
    }
    
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {                                       let identifier = tableViewCellReuseId
        ...
        //配置cell的Block
            ...
            guard let nonNilcell = cell else {
                assertionFailure("ERROR: cell can not be nil, please config cell with configureTableViewCellBlock")
                return UITableViewCell(frame: CGRectZero)
            }
            if shouldShowSearchResults{
                let newindexpath = NSIndexPath(forRow: indexPaths.objectAtIndex(indexPath.row) as! Int, inSection: 0)
                updateTableViewCellBlock(cell: nonNilcell, indexPath: newindexpath)
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        let fullTextVC = self.navigationController?.storyboard?.instantiateViewControllerWithIdentifier("fulltext") as! FullTextViewController
        fullTextVC.index = indexPath.row
        self.navigationController?.pushViewController(fullTextVC, animated: true)
        /*
        if(shouldShowSearchResults){
            selectedCell = newCellDictionary.valueForKey("\(indexPath.row)") as? UITableViewCell
        }
        */
    }
    ...
    func dismissPreview() {
        if(defaults.boolForKey("share")){
            let shareText = defaults.objectForKey("sharetext") as! String
